# Pi Container - Agent Guidelines

This repository contains a Fedora 44-based container image with [Pi](https://pi.dev/) cli tool pre-installed.
Pi ships as the `@earendil-works/pi-coding-agent` npm package; the Containerfile installs it globally with `npm install -g --ignore-scripts --min-release-age=0`.
...
make image        # Build container with latest pi version
make image VERSION=0.82.0  # Build specific pi version
```
...
### Testing
```bash
make test         # Run `pi --version` inside the pi container
```
...
```bash
./scripts/pi                      # Using the helper script (recommended)
podman run -it --security-opt label=disable --device /dev/fuse \
  --cap-add SYS_ADMIN,MKNOD localhost/pi:latest pi --version
```
...
# Constants in UPPERCASE: MAX_RETRIES=5
```
...
| Image names | lowercase-with-hyphens | `pi-container` |
...
- `${HOME}/.pi/agent` - Pi sessions, settings, prompts, skills, themes, extensions
- `${HOME}/.config/pi` - additional Pi user-level config
- `${HOME}/.local/share/pi` - Pi data directory (mirrors opencode's pattern)
- `${HOME}/.local/share/containers` - container storage for nested buildah/podman
